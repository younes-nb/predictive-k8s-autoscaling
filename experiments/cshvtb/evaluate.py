...
from experiments.cshvtb.dataset import CshvtbDataset
from experiments.cshvtb.model import CshvtbModel
...
def setup_logging(out_dir: str) -> str:
    ...
    log_path = os.path.join(out_dir, "evaluate_cshvtb.log")
...
def load_cshvtb_model(ckpt_path: str, device: torch.device) -> CshvtbModel:
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Evaluate CSHVTB model (TCN + BiGRU)."
    )
    ap.add_argument("--preprocess_dir", default="/dataset/cshvtb_preprocess")
    ap.add_argument("--model_dir", default="/proj/k8sautoscaledl-PG0/models/cshvtb")
    ...
    logging.info("Evaluating CSHVTB on %s", device)

    ckpt = os.path.join(args.model_dir, "cshvtb.pt")
    if not os.path.exists(ckpt):
        logging.error("Checkpoint not found: %s — train the model first.", ckpt)
        sys.exit(1)

    model = load_cshvtb_model(ckpt, device)
    logging.info("Loaded CSHVTB model from %s", ckpt)

    test_ds = CshvtbDataset(
    ...
    logging.info("")
    logging.info("=" * 60)
    logging.info("CSHVTB — Test Results  (n=%d samples)", n)
    logging.info("=" * 60)
