...
def setup_logging(out_dir: str) -> str:
    ...
    log_path = os.path.join(out_dir, "train_cshvtb.log")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Train CSHVTB model (TCN + BiGRU)."
    )
    ap.add_argument("--preprocess_dir", default="/dataset/cshvtb_preprocess")
    ap.add_argument("--out_dir", default="/proj/k8sautoscaledl-PG0/models/cshvtb")
    ...
    logging.info("=" * 60)
    logging.info("CSHVTB — Model Training on %s", device)
    logging.info("Log file: %s", log_path)
    logging.info("=" * 60)

    train_ds = CshvtbDataset(
    ...
    model = CshvtbModel(in_channels=6, cfg=CFG).to(device)
    ...
    ckpt_path = os.path.join(args.out_dir, "cshvtb.pt")
            ...
            torch.save(
                {
                    ...
                    "cfg": {
                        ...
                        "weight_init_low": CFG.WEIGHT_INIT_LOW,
                        "weight_init_high": CFG.WEIGHT_INIT_HIGH,
                    },
                },
                ckpt_path,
            )
    ...
    logging.info(
        "=== CSHVTB training complete. Best val loss: %.8f | Saved: %s ===",
