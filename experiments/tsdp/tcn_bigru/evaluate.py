...
def load_model(ckpt_path: str, device: torch.device, is_main: bool = False):
    ...
    model = TcnBiGru(
        ...
        tcn_kernel_size=cfg.get("tcn_kernel_size", ARCH_CFG.TCN_KERNEL_SIZE),
        tcn_filters=cfg.get("tcn_filters", ARCH_CFG.TCN_FILTERS),
        tcn_dilations=cfg.get("tcn_dilations", ARCH_CFG.TCN_DILATIONS),
        tcn_dropout=cfg.get("tcn_dropout", ARCH_CFG.TCN_DROPOUT),
        bigru_hidden=cfg.get("bigru_hidden", ARCH_CFG.BIGRU_HIDDEN),
    ).to(device)
