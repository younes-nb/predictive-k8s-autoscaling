...
def setup_logging(out_dir: str) -> None:
    ...
    sh.setFormatter(fmt)
    root.addHandler(sh)
    logging.getLogger("experiments.sdtnet.decomposition").setLevel(logging.WARNING)
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Decompose MSResource CPU signals into 6 SDTNet channels."
    )
    ap.add_argument("--msresource_dir", default=PATHS.PARQUET_MSRESOURCE)
    ap.add_argument("--out_dir", default="/dataset/sdtnet_preprocess")
    ...
    for i in range(total):
        dm = os.path.join(args.out_dir, "mode_0", f"service_{i:05d}.done")
    ...
    for batch_idx in range(n_batches):
        ...
        for ms_name, sig in signals.items():
            ...
            done_marker = os.path.join(args.out_dir, "mode_0", f"service_{idx:05d}.done")
        ...
        for ms_name in batch_services:
            if ms_name not in signals:
                ...
                if idx >= 0:
                    dm = os.path.join(args.out_dir, "mode_0", f"service_{idx:05d}.done")
