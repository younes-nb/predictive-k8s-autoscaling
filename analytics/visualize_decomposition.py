...
TSDP:
  - Image 1: Original signal + MODWT components (D1, D2, D3, A3)
  - Image 2: D1 + its VMD modes
  - Image 3: All output channels (VMD modes + D2 + D3 + A3)
"""
...
def find_service_with_window(preprocess_dir: str, max_services: int = 2000) -> tuple:
    """Scan services 0..max_services and return (idx, window_start, signal) of the service
    with the most transitions in a 60-step window containing >= 10 spikes > 0.8."""
    best = None
...
def ceemdan_decompose_and_plot(fig, signal, window_start=0):
    """Original + CEEMDAN IMFs only (no residue). SE annotation, K-means coloring."""
...
def plot_vmd_modes(fig, vmd_modes, input_signal, title="VMD Modes"):
    """Plot VMD mode components with the input signal they decompose."""
...
def plot_channels(fig, channels, titles, colors, suptitle):
    """Plot a list of 1D channel arrays as stacked subplots."""
    n = len(channels)
...
def get_modwt_components(signal, window_start=0):
    """
    Returns correctly-ordered MODWT components.

    pywt.mra returns [A3, D3, D2, D1] for level=3 (approximation first,
    details from coarsest to finest).
    """
    import pywt
    w = signal[window_start : window_start + 60].astype(np.float64)
    pad_len = 64 - len(w)
    padded = np.pad(w, (pad_len, 0), mode="edge") if pad_len > 0 else w
    coeffs = pywt.mra(padded, "sym4", level=3, transform="swt")
    if pad_len > 0:
        coeffs = [c[pad_len:] for c in coeffs]
    return coeffs[0], coeffs[1], coeffs[2], coeffs[3]


def plot_modwt_figure(fig, signal, window_start=0):
    """Original + MODWT components (correctly labeled)."""
    A3, D3, D2, D1 = get_modwt_components(signal, window_start)

    components = [
        ("D1 (Finest Detail — highest frequency)", D1, "#FF6B6B"),
        ("D2", D2, "#4ECDC4"),
        ("D3 (Coarsest Detail)", D3, "#45B7D1"),
        ("A3 (Approximation — lowest frequency)", A3, "#9B59B6"),
    ]

    gs = fig.add_gridspec(5, 1, height_ratios=[1] + [1] * 4, hspace=0.6)
...
def main():
    ...
    ap.add_argument("--service_idx", type=int, default=-1,
                    help="Service index (-1 = auto-select first service with >=10 spikes >0.8)")
    ...
    if args.service_idx < 0:
        ...
        args.service_idx = selected_idx
        tsdp_sig = load_service_signal(args.tsdp_preprocess_dir, args.service_idx)
    else:
        print(f"Loading service {args.service_idx}...")
        cvcbm_sig = load_service_signal(args.cvcbm_preprocess_dir, args.service_idx)
        tsdp_sig = load_service_signal(args.tsdp_preprocess_dir, args.service_idx)
        window_start = find_window_with_spikes(cvcbm_sig)
        if window_start is None:
            print("WARNING: No window with 10+ spikes > 0.8. Trying min_spikes=5...")
            window_start = find_window_with_spikes(cvcbm_sig, min_spikes=5)
        if window_start is None:
            print("WARNING: No suitable window found. Using index 0.")
            window_start = 0

    tsdp_start = min(window_start, len(tsdp_sig) - 60)
    w = cvcbm_sig[window_start:window_start+60]
    ...
    if imfs.shape[0] > 0:
        ...
        fig.suptitle(f"CVCBM — CEEMDAN Decomposition (Service {args.service_idx:05d})",
                     fontsize=14, fontweight="bold")
        p = os.path.join(args.out_dir, f"cvcbm_{args.service_idx:05d}_01_ceemdan.png")
        ...
        p = os.path.join(args.out_dir, f"cvcbm_{args.service_idx:05d}_02_vmd.png")
        plt.savefig(p, dpi=150, bbox_inches="tight")
        print(f"    Saved: {p}")
        if args.show:
            plt.show()
        ...
        p = os.path.join(args.out_dir, f"cvcbm_{args.service_idx:05d}_03_outputs.png")
        plt.savefig(p, dpi=150, bbox_inches="tight")
        print(f"    Saved: {p}")
        if args.show:
            plt.show()
    ...
    print("  Computing MODWT + VMD in-memory...")
    A3, D3, D2, D1 = get_modwt_components(tsdp_sig, window_start=tsdp_start)
    ...
    plot_modwt_figure(fig, tsdp_sig, window_start=tsdp_start)
    fig.suptitle(f"TSDP — MODWT Decomposition (Service {args.service_idx:05d})",
                 fontsize=14, fontweight="bold")
    p = os.path.join(args.out_dir, f"tsdp_{args.service_idx:05d}_01_modwt.png")
    plt.savefig(p, dpi=150, bbox_inches="tight")
    print(f"    Saved: {p}")
    if args.show:
        plt.show()
    ...
    p = os.path.join(args.out_dir, f"tsdp_{args.service_idx:05d}_02_vmd.png")
    plt.savefig(p, dpi=150, bbox_inches="tight")
    print(f"    Saved: {p}")
    if args.show:
        plt.show()
    ...
    fig = plt.figure(figsize=(14, 2.5 * (n_vmd + 3)))
    vmd_colors = plt.cm.tab10(np.linspace(0, 1, n_vmd))
    channels = [tsdp_vmd_modes[k] for k in range(n_vmd)] + [D2, D3, A3]
    titles = [f"VMD Mode {k+1} (from D1)" for k in range(n_vmd)] + [
        "D2", "D3", "A3 (Approximation)",
    ]
    colors = list(vmd_colors) + ["#4ECDC4", "#45B7D1", "#9B59B6"]
    plot_channels(fig, channels, titles, colors, "TSDP — All 13 Output Channels")
    p = os.path.join(args.out_dir, f"tsdp_{args.service_idx:05d}_03_outputs.png")
    plt.savefig(p, dpi=150, bbox_inches="tight")
    print(f"    Saved: {p}")
    if args.show:
        plt.show()
