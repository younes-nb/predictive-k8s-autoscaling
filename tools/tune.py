...
def get_service_baselines():
    ...
    for x_path in x_files[:5]:
        ...
        if not os.path.exists(sid_path): continue
        X = np.load(x_path, mmap_mode="r")
        SIDs = np.load(sid_path, mmap_mode="r")
        u_now = X[:, -1, -1] if X.ndim == 3 else X[:, -1]
        for i in range(len(SIDs)):
            s = int(SIDs[i]); (service_vals.setdefault(s, [])).append(u_now[i])
    return {s: np.quantile(v, TRAINING.THETA_BASE) for s, v in service_vals.items()}

SERVICE_BASELINES = get_service_baselines()


train_ds = ShardedWindowsDataset(PATHS.WINDOWS_DIR, "train", PREPROCESSING.INPUT_LEN, PREPROCESSING.PRED_HORIZON)
val_ds = ShardedWindowsDataset(PATHS.WINDOWS_DIR, "val", PREPROCESSING.INPUT_LEN, PREPROCESSING.PRED_HORIZON)
...
def objective(trial):
    ...
    delta = trial.suggest_float("delta", 0.01, 0.10) 
    under_penalty = trial.suggest_float("under_penalty", 2.0, 15.0)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    train_loader = DataLoader(train_ds, batch_size=TRAINING.BATCH_SIZE, shuffle=True, num_workers=4)
    val_loader = DataLoader(val_ds, batch_size=TRAINING.BATCH_SIZE, shuffle=False, num_workers=4)
    ...
    for epoch in range(TRAINING.EPOCHS):
        ...
        for x, y, sid in train_loader:
            ...
            w = get_ground_truth_weights(y, sid, device, gamma, delta, TRAINING.THETA_MIN, TRAINING.THETA_MODE)
            
            optimizer.zero_grad()
            loss = weighted_mse(model(x), y, w, under_penalty=under_penalty)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for x, y, sid in val_loader:
                x, y = x.to(device), y.to(device)
                w_val = get_ground_truth_weights(y, sid, device, gamma, delta, TRAINING.THETA_MIN, TRAINING.THETA_MODE)
                loss_val = weighted_mse(model(x), y, w_val, under_penalty=under_penalty)
                
                batch_w_sum = w_val.sum().item()
                total_val_loss += loss_val.item() * batch_w_sum
                total_w += batch_w_sum

        avg_val = total_val_loss / (total_w + 1e-6)
